...
function buildLyricId(docId: string | undefined, lineIndex: number | undefined, text: string): string {
...
function toChromaLyricsJSONLines(inputJsonl: string, producedIds: Set<string>) {
	...
	for (const line of lines) {
		...
		const metadata: Record<string, unknown> = {
			...
			tonePatternStr: groupTonePatternByTokens(obj.prosody?.tone_pattern ?? [], obj.nlp?.tokens ?? []),
			jyutping: (obj.prosody?.tone_pattern_cantonese_jyutping ?? []).join(" "),
			syllables: (obj.prosody?.tone_pattern_cantonese_jyutping ?? []).length || obj.structure?.char_count,
