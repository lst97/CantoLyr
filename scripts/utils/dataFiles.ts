    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        for (const it of parsed as any[]) {
          ...
          if (typeof w === 'string' && typeof c === 'number' && Number.isFinite(c)) pairs.push({ w, c });
        }
...
export function loadDetailedSentimentMaps(filePath: string): { regMap: Map<string, string>; posMap: Map<string, string> } {
  const raw = readFileSync(filePath, 'utf-8');
