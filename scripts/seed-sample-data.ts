	...
	try {
		...
		const existingCount = await prisma.entry.count({
			where: {
				readings: {
					some: {
						source: { in: ["words_hk_v28042025", "words_hk_v28042025"] },
					},
				},
			},
		});
		...
		if (existingCount > 0) {
			console.log(
				`⚠️  Found ${existingCount} existing entries with sample data sources.`
			);
		...
		for (const entry of allEntries) {
			try {
				...
				if (insertedEntries % 1000 === 0) {
					console.log(
						`📈 Progress: ${insertedEntries}/${allEntries.length} entries inserted...`
					);
				}
			} catch (error) {
