...
type LyricLineData = {
	...
	context_links: { doc_id: string; prev_line_id?: string | null; next_line_id?: string | null; paragraph_id?: string };
};

function parseJyutping(syllable: string): { consonant: string | null; rhyme: string | null; toneRaw: number | null; toneDigit: number | null } {
	...
	const toneDigit =
		toneRaw && pinyinTable.tones[String(toneRaw) as keyof typeof pinyinTable.tones]
...
async function main() {
	...
	if (await fileExists(charInput)) {
		...
		const glossMap = await fileExists(charDetail)
	...
	if (await fileExists(wordInput)) {
		...
		const glossMap = await fileExists(wordDetail)
...
async function bulkInsertLyricsData(prisma: PrismaClient, batch: LyricLineData[]) {
	...
	for (const d of batch) {
		...
		if (!songByDoc.has(doc)) songByDoc.set(doc, { title: d.source.title, year: d.source.year ?? null });
	}
	...
	const [artists, lyricists, themes, keywords] = await Promise.all([
		(prisma as any).artist.findMany({ where: { name: { in: Array.from(artistSet) } }, select: { id: true, name: true } }),
		(prisma as any).lyricist.findMany({ where: { name: { in: Array.from(lyricistSet) } }, select: { id: true, name: true } }),
		(prisma as any).theme.findMany({ where: { name: { in: Array.from(themeSet) } }, select: { id: true, name: true } }),
		(prisma as any).keyword.findMany({ where: { word: { in: Array.from(keywordSet) } }, select: { id: true, word: true } }),
	]);
	...
	const songRows = Array.from(songByDoc.entries()).map(([docId, { title, year }]) => ({ docId, title, year }));
	...
	const songs = await (prisma as any).song.findMany({ where: { docId: { in: songRows.map((s) => s.docId) } }, select: { id: true, docId: true } });
	...
	const lyricRowsInserted = await (prisma as any).lyricLine.findMany({ where: { lyricId: { in: lyricIds } }, select: { id: true, lyricId: true } });
	...
	// 9) Syllables
	const syllableRows: Array<{ lyricId: bigint; position: number; jyutping: string; consonant: string | null; rhyme: string | null; toneRaw: number | null; toneDigit: number | null }>= [];
	for (const d of batch) {
		const lid = lyricIdByLyricKey.get(d.id);
		if (!lid) continue;
	...
	const ngramRows: Array<{ lyricId: bigint; n: number; value: string; position: number; syllableCount: number; tokenCount: number }>= [];
	for (const d of batch) {
		const lid = lyricIdByLyricKey.get(d.id);
		if (!lid) continue;
		const toneDigits = d.prosody.tone_pattern;
		...
		for (let i = 0; i < toneDigits.length; i++) {
			ngramRows.push({ lyricId: lid, n: 1, value: String(toneDigits[i]), position: i + 1, syllableCount, tokenCount });
		}
		for (let i = 0; i < toneDigits.length - 1; i++) {
			ngramRows.push({ lyricId: lid, n: 2, value: `${toneDigits[i]}${toneDigits[i + 1]}`, position: i + 1, syllableCount, tokenCount });
		}
	}
