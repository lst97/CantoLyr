 ...
 */
export function normalizeWordslistData(data: WordslistData, source: string = 'wordslist_v1'): WordslistEntry[] {
  ...
  for (const [word, jyutpingArray] of Object.entries(data)) {
    ...
    for (const jyutping of jyutpingArray) {
      try {
        ...
        console.warn(`Failed to normalize reading "${jyutping}" for word "${word}":`, error);
        continue;
      }
    }
 ...
 * @returns JSONL string
 */
export function processWordslistToJSONL(data: WordslistData, source: string = 'wordslist_v1'): string {
