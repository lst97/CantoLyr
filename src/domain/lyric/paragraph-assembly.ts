...
function computeCoherence(
  prev: SentenceCandidate | undefined,
  current: SentenceCandidate,
): number {
  // Use continuityScore if provided else heuristic with relevanceScore
  const base = safeNormalizeScore(
    current.continuityScore ?? current.relevanceScore ?? 0.5,
  );
  if (!prev) return base;
  const delta = Math.abs(
    (prev.emotionalArcScore ?? 0.5) - (current.emotionalArcScore ?? 0.5),
  );
...
function computeDiversity(sentences: SentenceCandidate[]): number {
  ...
  const sum = sentences.reduce(
    (acc, s) =>
      acc +
      safeNormalizeScore(s.diversityScore ?? (1 - (s.diversityPenalty ?? 0))),
    0,
  );
...
function computeEmotionalArc(sentences: SentenceCandidate[]): number {
  ...
  const variance = scores.reduce((a, b) => a + (b - mean) ** 2, 0) /
    scores.length;
...
function extendBeam(node: BeamNode, candidate: SentenceCandidate): BeamNode {
  ...
  const coherence = (node.coherence * node.sentences.length +
    computeCoherence(prev, candidate)) /
...
export function assembleParagraphs(
  ...
  for (const line of sortedLines) {
    ...
    next.sort((a, b) =>
      b.final - a.final ||
      a.sentences.map((s) => s.id).join("").localeCompare(
        b.sentences.map((s) => s.id).join(""),
      )
    );
