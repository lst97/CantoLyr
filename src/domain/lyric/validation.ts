...
export function validateSegmentationPattern(pattern: SegmentationPattern): void {
...
export function validateParagraphVariant(p: ParagraphVariant): void {
  ...
  assert(p.finalScore >= 0 && p.finalScore <= 1, "ParagraphVariant finalScore out of range");
}

export function safeNormalizeScore(value: number | undefined, fallback = 0): number {
