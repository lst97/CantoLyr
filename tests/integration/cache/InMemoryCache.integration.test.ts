...
describe('InMemoryCache Integration', () => {
  ...
  describe('Compose Result Caching', () => {
    ...
    it('should handle compose cache with different topK values', async () => {
      ...
      await cache.set(InMemoryCache.generateComposeKey(toneMap, 10), baseResults);
      await cache.set(InMemoryCache.generateComposeKey(toneMap, 20), baseResults);
      await cache.set(InMemoryCache.generateComposeKey(toneMap), baseResults); // default

      // Verify separate caching
      const result10 = await cache.get(InMemoryCache.generateComposeKey(toneMap, 10));
      const result20 = await cache.get(InMemoryCache.generateComposeKey(toneMap, 20));
      const resultDefault = await cache.get(InMemoryCache.generateComposeKey(toneMap));
  ...
  describe('Cache Warming and Invalidation Scenarios', () => {
      ...
      const frequentPatterns = [
        {
          ...
          value: [{ id: BigInt(1), surface: '測試', toneMapped: '34' }],
          ttl: 120,
        },
        {
          key: InMemoryCache.generateSearchKey({ v: '403' }),
          value: [{ id: BigInt(2), surface: '債權人', toneMapped: '403' }],
          ttl: 120,
        },
        {
          key: InMemoryCache.generateSearchKey({ v: '0' }),
          value: [{ id: BigInt(3), surface: '亡', toneMapped: '0' }],
          ttl: 120,
        },
      ];

      await cache.warmCache(frequentPatterns);
    ...
    it('should invalidate search cache when data changes', async () => {
      ...
      await cache.set(InMemoryCache.generateSearchKey({ v: '34' }), ['result1']);
      await cache.set(InMemoryCache.generateSearchKey({ v: '403' }), ['result2']);
      ...
      expect(await cache.get(InMemoryCache.generateSearchKey({ v: '34' }))).toBeNull();
      expect(await cache.get(InMemoryCache.generateSearchKey({ v: '403' }))).toBeNull();
      expect(await cache.get(InMemoryCache.generateComposeKey('34'))).not.toBeNull();
    });
    ...
    it('should handle cache invalidation for specific tone patterns', async () => {
      ...
      for (const pattern of patterns) {
        await cache.set(
          InMemoryCache.generateSearchKey({ v: pattern }),
          [`results-${pattern}`]
        );
      }
      ...
      expect(await cache.get(InMemoryCache.generateSearchKey({ v: '34' }))).toBeNull();
      expect(await cache.get(InMemoryCache.generateSearchKey({ v: '340' }))).toBeNull();
      expect(await cache.get(InMemoryCache.generateSearchKey({ v: '3403' }))).toBeNull();
      expect(await cache.get(InMemoryCache.generateSearchKey({ v: '403' }))).not.toBeNull();
    });
  });
  ...
  describe('Performance and Memory Management', () => {
    it('should handle large result sets efficiently', async () => {
      ...
      const largeResultSet: ReadingDTO[] = Array.from({ length: 1000 }, (_, i) => ({
        id: BigInt(i + 1),
        entryId: BigInt(i + 1),
        surface: `字${i}`,
        type: 'char' as const,
  ...
  describe('Real-world Usage Patterns', () => {
    it('should simulate typical search workflow with caching', async () => {
      ...
      const composeResults = {
        ranking: [
          { id: BigInt(1), score: 0.95, reason: 'Perfect tone match' },
        ],
      };
