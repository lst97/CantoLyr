...
import { ComposeLineUseCase, type ComposeLineInput } from '../../../../src/application/use-cases/ComposeLineUseCase.js';
...
import type { LlmGroupedSelector, GroupedSelectionResult } from '../../../../src/application/ports/LlmGroupedSelector.js';
...
describe('ComposeLineUseCase', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    composeLineUseCase = new ComposeLineUseCase(mockReadingRepo, mockCache, mockLlmGroupedSelector);
  });
  ...
  describe('execute', () => {
    it('should return cached results when available', async () => {
      ...
      expect(mockCache.get).toHaveBeenCalledWith('compose:4_0:250:none:none:none:zh-HK:random');
    ...
    it('should execute compose workflow with LLM when cache miss', async () => {
      ...
      vi.mocked(mockLlmGroupedSelector.selectFromGroups).mockResolvedValue(llmResult);
      ...
      expect(result.reason).toBe('Selected words that convey sadness about debt');
    ...
    it('should fallback to heuristic selection when LLM fails', async () => {
      ...
      expect(result.reason).toBe('Fallback to highest frequency selection per group');
    ...
    it('should require tone pattern', async () => {
      ...
      // Act & Assert
      await expect(composeLineUseCase.execute(input)).rejects.toThrow('Tone pattern is required');
    });
    ...
    it('should generate correct cache keys', async () => {
      ...
      // Assert
      expect(mockCache.get).toHaveBeenCalledWith('compose:4_0_3:100:love:happy:pop:zh-CN:42');
    });
    ...
    it('should pass fetch function to prefilter correctly', async () => {
      ...
      vi.mocked(mockReadingRepo.searchByToneMapped).mockResolvedValue(sampleReadings);
