...
import { SearchUseCase, type SearchInput } from '../../../../src/application/use-cases/SearchUseCase.js';
...
describe('SearchUseCase', () => {
  ...
  describe('execute', () => {
    ...
    it('should handle prefix search correctly', async () => {
      ...
      vi.mocked(mockCache.get).mockResolvedValue(null);
      vi.mocked(mockReadingRepo.searchByToneMapped).mockResolvedValue([sampleReadings[0]!]);
    ...
    it('should require tone pattern', async () => {
      ...
      // Act & Assert
      await expect(searchUseCase.execute(input)).rejects.toThrow('Tone pattern is required');
    });
    ...
    it('should generate correct cache keys for different parameters', async () => {
      ...
      const inputs: SearchInput[] = [
        ...
        { tonePattern: '403', isPrefix: true, entryType: 'vocab', limit: 100, offset: 10 }
      ];
      ...
      // Assert
      expect(mockCache.get).toHaveBeenCalledWith('search:403:exact:all:50:0');
      expect(mockCache.get).toHaveBeenCalledWith('search:403:prefix:vocab:100:10');
    });
